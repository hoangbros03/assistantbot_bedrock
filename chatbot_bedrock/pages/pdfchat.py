...
def new_text_file():
    ...
    # Get embed usage
    demo.get_current_token_usage(st.session_state.token_status_obj, None, st.session_state.embed_model)

uploaded_text_file = st.file_uploader("Choose a file", type=['txt', 'pdf'], key=f"uploader_{str(st.session_state.text_uploader_key)}", on_change=new_text_file)

input_text = st.chat_input("Start chat with pdf...", disabled=bool(uploaded_text_file is None))
...
if input_text:
    ...
    ref_list ='\n'.join([f"Page {doc.metadata['page']}: {doc.page_content}" for doc in ref_list])
    ref_part = f"References found when retrieve data:\n{ref_list}"
    with st.chat_message('assistant'):
        st.markdown(f"{ref_part[:50]}...")
    print("Ref part: ", ref_part[:50])
    st.session_state.memory.save_context({"input": "Found relevant information to answer the last question"}, {"output": ref_part})
