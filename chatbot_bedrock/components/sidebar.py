...
def sidebar():
    ...
    st.sidebar.download_button("Export the chat", data="\n".join([f"role: {i['role']}, content: {i['text']}" for i in st.session_state.chat_history]), file_name="chat_history.txt")

    select_event = st.sidebar.selectbox('Select model',
                                        [ "Amazon Titan Text Premier", "Claude 3.5 Sonnet",'Claude 3 Haiku','Amazon Titan Text Express'])
